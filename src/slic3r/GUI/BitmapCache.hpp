...
public:
    ...
    // Load an SVG from an absolute file path (plugin-supplied icon), rasterized to the given size.
    wxBitmap 		load_svg_from_path(const std::string &abs_path, unsigned width, unsigned height);
